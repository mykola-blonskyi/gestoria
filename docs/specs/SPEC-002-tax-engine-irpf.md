...
```
`ReduccionTrabajo` piecewise per `config.trabajo.reduccion` (thresholds t1/t2/t3 = 14,852 / 17,673.52 / 19,747.50; coefficients k1/k2 = 1.75 / 1.14; fixed 7,302 for 2025). It is measured on `rnPrevio`, **before** the 2,000 of otros gastos: LIRPF art. 20 as in force for 2025 defines the rendimiento neto for this purpose as the íntegro less the expenses of art. 19.2 a) to e), and the AEAT Manual práctico Renta 2025 (cap. 3, fase 3) repeats it. **Guard:** if `otrasRentas > config.trabajo.reduccion.otherIncomeCap` (6,500) → 0 (golden #8). `otrasRentas` = all non-employment income (activity net, savings gross, rental net, imputed).
