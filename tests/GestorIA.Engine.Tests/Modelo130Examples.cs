...
public class Modelo130Examples
{
    ...
    public void PaymentIsTheRateOnTheBaseLessRetencionesEarlierPaymentsAndMinoracion()
    {
        ...
        // 0.20 × 20,000 − 1,500 − 600 − 75
        Assert.Equal(new Money(1825m), result.Resultado);
    ...
    public void PendingNegativesAreDeductedOnlyUpToAPositiveResult()
    {
        ...
        Assert.Equal(200m, Step(result, "m130.negativos-anteriores").Output);
        Assert.Equal(Money.Zero, result.Resultado);
        Assert.Equal(new Modelo130Carry(new Money(1200m), new Money(300m)), result.Carry);
    }
    ...
    [Fact]
    public void DificilJustificacionIsNotAppliedWhenTheConfigSaysNot()
    {
        Assert.False(TaxYearConfigFiles.Year2025.Modelo130.ApplyDj);

        var result = Run(ingresos: 50000m, gastos: 7600m);

        Assert.Equal(0m, Step(result, "m130.dificil-justificacion").Output);
        Assert.Equal(42400m, Step(result, "m130.rendimiento-neto").Output);
    }

    [Fact]
    public void DificilJustificacionIsAppliedWhenTheConfigSaysSo()
    {
        var config = TaxYearConfigFiles.Year2025;
        var withDj = config with { Modelo130 = config.Modelo130 with { ApplyDj = true } };

        var result = Run(ingresos: 50000m, gastos: 7600m, config: withDj);

        Assert.Equal(2000m, Step(result, "m130.dificil-justificacion").Output);
        Assert.Equal(new Money(8080m), result.Resultado);
    }
    ...
    public void MinoracionReducesEveryQuarterAndIsNotClawedBackByTheNext()
    {
        ...
        Assert.Equal(new Money(900m), q1.Resultado);
        Assert.Equal(new Money(900m), q2.Resultado);
    }
